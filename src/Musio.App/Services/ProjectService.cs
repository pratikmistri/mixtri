...
public class ProjectService
{
    ...
    /// <summary>
    /// Inserts an external audio file (already normalised by <see cref="AudioImportService"/>
    /// into a PCM WAV) into the current project as a positioned <see cref="AudioTrack"/>.
    /// </summary>
    /// <remarks>
    /// Unlike <see cref="ImportVideo"/> this never creates a project: an audio track has no
    /// frames, so there is nothing for it to be the primary recording of. Importing audio
    /// with no project open is therefore a no-op returning <c>null</c>, which the caller
    /// surfaces rather than silently producing an empty timeline.
    /// </remarks>
    /// <param name="result">The normalised audio produced by the import.</param>
    /// <param name="kind">Whether this was inserted as narration or as a music bed.</param>
    /// <param name="startTime">Where it lands on the output timeline (typically the playhead).</param>
    /// <returns>The inserted track, or <c>null</c> when there was no project to insert into.</returns>
    public AudioTrack? ImportAudio(AudioImportResult result, AudioTrackKind kind, TimeSpan startTime)
    {
        ...
        var track = new AudioTrack
        {
            ...
            Name = TrackNameFromImport(result, kind),
            Kind = kind,
    ...
    /// <summary>
    /// Derives a track name from the imported file's original name, falling back to the
    /// kind's own label when the source name is unavailable.
    /// </summary>
    private static string TrackNameFromImport(AudioImportResult result, AudioTrackKind kind)
    {
        var name = Path.GetFileNameWithoutExtension(result.SourceFileName);
        if (!string.IsNullOrWhiteSpace(name)) return name;
        return kind == AudioTrackKind.Music ? "Music" : "Voice over";
    }

    /// <summary>
    /// Removes an inserted audio track by id. Returns whether anything was removed.
    /// </summary>
    /// <remarks>
    /// The normalised WAV in the app-owned import folder is deliberately left on disk: undo
    /// has to be able to bring the track back, and the orphan sweep reclaims import folders
    /// no saved project references.
    /// </remarks>
    public bool RemoveAudioTrack(Guid trackId)
    {
        if (CurrentProject is null) return false;

        int removed = CurrentProject.AudioTracks.RemoveAll(t => t.Id == trackId);

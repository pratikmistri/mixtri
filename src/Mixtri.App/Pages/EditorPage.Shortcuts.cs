...
public sealed partial class EditorPage
{
    ...
    private async void ImportVideo_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Mixtri.Core.Diagnostics.DiagLog.Write("Editor", $"Import picker failed: {ex.Message}");
        }
    ...
    private async Task<VideoSegment?> ImportVideoFileAsync(string path, TimeSpan insertAt)
    {
        ...
        {
            Mixtri.Core.Diagnostics.DiagLog.Write("Editor", $"Video import failed: {ex}");
            errorMessage = ex.Message;
        }
        finally
        {
    ...
    private async Task ImportAudioAsync(AudioTrackKind kind)
    {
        ...
        {
            Mixtri.Core.Diagnostics.DiagLog.Write("Editor", $"Audio import picker failed: {ex.Message}");
        }
        if (file is null) return;
    ...
    private async Task ImportAudioFileAsync(string path, AudioTrackKind kind, TimeSpan startTime)
    {
        ...
        {
            Mixtri.Core.Diagnostics.DiagLog.Write("Editor", $"Audio import failed: {ex}");
            errorMessage = ex.Message;
        }
        finally
        {

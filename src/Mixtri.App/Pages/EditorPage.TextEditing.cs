...
public sealed partial class EditorPage
{
    ...
    private void BuildGradientPresetsIfNeeded()
    {
        ...
        foreach (var preset in Mixtri.Core.Settings.DefaultBrandPresets.All)
        {
    ...
    {
        if (_suppressSlideEvents) return;
        if (SlideGradientPresets.SelectedItem is not Border { Tag: Mixtri.Core.Settings.BrandPreset preset }) return;
        var slide = SelectedSlide();
        if (slide is null) return;
    ...
    /// <see cref="Mixtri_App.Controls.RegionSelectorOverlay"/>'s handles) around the box,
    ...
    /// <see cref="Mixtri_App.Controls.RegionSelectorOverlay"/>'s GetCursorForHandle.</summary>
    ...
    /// <see cref="Mixtri_App.Controls.RegionSelectorOverlay"/>'s ApplyResize: a corner moves
    ...
    private interface ITextEditTarget
    {
        ...
        /// snaps it to <see cref="Mixtri.Core.Timeline.TextOverlayAnchor.Custom"/> at its

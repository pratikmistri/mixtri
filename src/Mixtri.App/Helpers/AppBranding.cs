...
/// </summary>
/// <remarks>
/// A locally deployed dev build is labelled <c>"Mixtri (dev)"</c> so it is instantly
/// distinguishable from the Store build, which stays plain <c>"Mixtri"</c>. It also carries a
/// separate package identity so the two install side by side. Both are applied to the generated
/// <c>AppxManifest.xml</c> by the <c>ApplyMixtriDevBranding</c> target in <c>Mixtri.App.csproj</c>
...
internal static class AppBranding
{
    ...
    /// <summary>"Mixtri" for the Store build, "Mixtri (dev)" for a local dev deployment.</summary>
    public static string DisplayName => LazyDisplayName.Value;

    private const string Fallback = "Mixtri";

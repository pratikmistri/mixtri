...
internal abstract class OverlayWindowBase
{
        ...
        where T : class
    {
        ...
        var shell = Mixtri_App.Services.ShellCoordinator.Instance;
        var mainWindow = Mixtri_App.App.Current.MainAppWindow;

        try
        {
            // Minimize Mixtri so it doesn't appear in the screenshot.
        ...
        finally
        {
            ...
            // Restore Mixtri if we minimized it
            if (didMinimize)
            {
                if (shell is not null)
                {

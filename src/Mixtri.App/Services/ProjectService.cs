...
public class ProjectService
{
    ...
    /// <summary>
    /// True when the project has edits that have not been written to its <c>.mixtri</c> file.
    ...
    /// either it has edits that are not in its <c>.mixtri</c> file, or it has no file at all.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Path of the <c>.mixtri</c> file this project was opened from or last saved to,
    ...
    /// <summary>
    /// Folder that <c>.mixtri</c> packages unpack their media into.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Opens a <c>.mixtri</c> package and makes it the current project.
    /// </summary>
        ...
        string packagePath, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        ...
        try
        {
            var result = await MixtriPackageService.OpenAsync(
            ...
            foreach (var path in MixtriPathRewriter.Enumerate(

...
/// </summary>
/// <remarks>
/// The <c>$kind</c> discriminators are part of the on-disk <c>.mixtri</c> format. Renaming
...
/// member; <c>MixtriPackageTests.Open_ToleratesLegacyPerSegmentTextOverlays</c> pins that,
/// so opting <see cref="MixtriPackage.JsonOptions"/> into strict member handling would fail
...
/// </para>
/// <para>
/// These values are serialized <b>by name</b> (<c>MixtriPackage.JsonOptions</c> registers a
...
public enum SegmentAudioMode
{
    ...
    /// (see <c>Mixtri.Core.Audio.WsolaTimeStretcher</c>), preserving pitch. The default, and
...
/// </summary>
/// <remarks>
/// These values are serialized <em>by name</em> (see <c>Mixtri.Core.Projects.MixtriPackage.JsonOptions</c>,
/// which registers a <c>JsonStringEnumConverter</c>), so every value here is part of the on-disk
/// <c>.mixtri</c> format. Existing members must never be reordered or renamed — doing so silently

...
// equal-power ramp implementation (Mixtri.Core.Audio.EqualPowerCrossfade applied via its
...
/// <see cref="Mixtri.Core.Audio.EqualPowerCrossfade"/>). <see cref="TimeSpan.Zero"/> (the
...
/// </para>
/// <para>
/// Below 1.0 only for an inserted <see cref="Mixtri.Core.Models.AudioTrack"/> (voice-over
...
/// (<see cref="Mixtri.Core.Audio.EqualPowerCrossfade"/>) instead of a linear one, which
...
/// project — a plain <c>Microsoft.NET.Sdk</c> class library like <c>Mixtri.Core</c> cannot
...
/// <c>Mixtri.App\Package.appxmanifest</c>. Both are structural, cross-project changes far
...
/// repository. No workaround inside <c>Mixtri.Core</c>/<c>Mixtri.App</c>'s existing project
...
/// cref="Mixtri.Core.Audio.AudioPlaybackEngine"/> DOES contain a working, tested
/// equal-power ramp (<c>EqualPowerFadeSampleProvider</c>, driven by the same
/// <see cref="Mixtri.Core.Audio.EqualPowerCrossfade"/> curve), and its
...
/// <see cref="Mixtri.Core.Audio.AudioPlaybackEngine"/>'s remarks for the complete
...
public static class ExportAudioPlan
{
    ...
    /// <see cref="Mixtri.Core.Audio.EqualPowerCrossfade"/>) should run, on both sides of

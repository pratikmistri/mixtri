...
public class VideoEncoder : IDisposable
{
    ...
    /// Component project (producing a <c>.winmd</c>) — <c>Mixtri.Core</c> is a plain
    ...
    /// <c>Mixtri.App\Package.appxmanifest</c>. Both are structural, cross-project changes —
    ...
    /// the pre-existing hard cut. Live preview (<see cref="Mixtri.Core.Audio.AudioPlaybackEngine"/>)
    ...
    /// <c>Mixtri.Tests</c> (via this assembly's <c>InternalsVisibleTo</c>) can assert the
    ...
    /// why — the ramp exists in <see cref="Mixtri.Core.Audio.AudioPlaybackEngine"/> but is
    ...
    /// <c>Mixtri.Tests</c> (via this assembly's <c>InternalsVisibleTo</c>) can assert

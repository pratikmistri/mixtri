...
/// <summary>
/// Shared constants and helpers for the <c>.mixtri</c> single-file project format.
/// </summary>
/// <remarks>
/// <para>
/// A <c>.mixtri</c> file is an ordinary ZIP archive. Windows Explorer has no concept of a
...
/// </para>
/// </remarks>
public static class MixtriPackage
{
    ...
    /// <summary>File extension the app writes, including the leading dot.</summary>
    public const string FileExtension = ".mixtri";

    /// <summary>
    /// Extensions that are still opened but never written, newest first.
    /// </summary>
    /// <remarks>
    /// <c>.musio</c> is the app's pre-rename extension. The format did not change with the
    /// rename — a <c>.musio</c> file is byte-identical to a <c>.mixtri</c> one — so this is
    /// purely a naming alias, not a compatibility shim, and no version gate belongs here.
    /// Read paths must accept every extension in this list (see <see cref="IsPackagePath"/>
    /// and <see cref="AllExtensions"/>); the save picker must offer only
    /// <see cref="FileExtension"/>, so a re-saved legacy project migrates to the new name.
    /// </remarks>
    public static readonly string[] LegacyFileExtensions = [".musio"];

    /// <summary>Every extension that names a project package, <see cref="FileExtension"/> first.</summary>
    public static readonly string[] AllExtensions = [FileExtension, .. LegacyFileExtensions];
    ...
    /// <summary>
    /// Returns true when <paramref name="path"/> names a Mixtri project package, under either
    /// the current extension or any legacy one.
    /// </summary>

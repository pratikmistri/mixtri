...
public sealed class InsertedAudioTrackTests
{
    ...
    {
        var placement = new Mixtri.Core.Audio.AudioTimelinePlacement(
            VoicePath,
    ...
    {
        var window = Mixtri.Core.Audio.WaveformWindow.Resolve(
    ...
    public void WaveformWindow_ScalesWithTheWindow_NotWithTheFile()
    {
        ...
        var early = Mixtri.Core.Audio.WaveformWindow.Resolve(1000, 100, 10, 5);
        var late = Mixtri.Core.Audio.WaveformWindow.Resolve(1000, 100, 80, 5);
    ...
    public void WaveformWindow_IsEmptyForUnusableInput_RatherThanThrowing()
    {
        ...
        Assert.IsTrue(Mixtri.Core.Audio.WaveformWindow.Resolve(100, 0, 0, 5).IsEmpty);
        Assert.IsTrue(Mixtri.Core.Audio.WaveformWindow.Resolve(0, 10, 0, 5).IsEmpty);
        Assert.IsTrue(Mixtri.Core.Audio.WaveformWindow.Resolve(100, 10, 0, 0).IsEmpty);
    }
    ...
    public void WaveformWindow_PeakSourceTime_IsIndependentOfTheWindow()
    {
        ...
        var wide = Mixtri.Core.Audio.WaveformWindow.Resolve(1000, 100, 0, 100);
        var narrow = Mixtri.Core.Audio.WaveformWindow.Resolve(1000, 100, 20, 5);
    ...
    private static Mixtri.Core.Audio.LaneBlock Block(string id, double startSec, double endSec)
        => new(id, TimeSpan.FromSeconds(startSec), TimeSpan.FromSeconds(endSec));

    [TestMethod]
    public void Pack_NonOverlappingBlocks_AllShareOneRow()
    {
        var rows = Mixtri.Core.Audio.AudioLaneLayout.PackIntoRows(
        ...
        Assert.AreEqual(1, Mixtri.Core.Audio.AudioLaneLayout.RowCount(rows));
    }
    ...
    public void Pack_OverlappingBlocks_GetSeparateRows()
    {
        ...
        Assert.AreEqual(2, Mixtri.Core.Audio.AudioLaneLayout.RowCount(rows));
    }
    ...
    public void Pack_UsesExactlyThePeakOverlapCount()
    {
        ...
        Assert.AreEqual(3, Mixtri.Core.Audio.AudioLaneLayout.RowCount(rows),
    ...
    {
        var forward = Mixtri.Core.Audio.AudioLaneLayout.PackIntoRows(
            [Block("a", 0, 10), Block("b", 5, 15), Block("c", 20, 25)], maxRows: 5);
        var reversed = Mixtri.Core.Audio.AudioLaneLayout.PackIntoRows(
    ...
    {
        var rows = Mixtri.Core.Audio.AudioLaneLayout.PackIntoRows([], maxRows: 5);
        Assert.AreEqual(1, Mixtri.Core.Audio.AudioLaneLayout.RowCount(rows),

...
public sealed class SessionCleanupServiceTests
{
    ...
    public void SetUp()
    {
        _testRoot = Path.Combine(Path.GetTempPath(), $"mixtri_cleanup_test_{Guid.NewGuid():N}");
        ...
        bool withMarker = false, long videoSize = 1024, bool finalized = true)
    {
        ...
        {
            File.WriteAllText(
                Path.Combine(dir, Mixtri.Core.Capture.VideoWriter.FinalizedMarkerName),
                $"{{\"encoderVersion\":{Mixtri.Core.Capture.VideoWriter.EncoderVersion}}}");
        }
        ...
        string? explicitName = null, long payloadSize = 4096)
    {
        ...
        if (withVideo)
        {
            ...
            File.WriteAllText(
                Path.Combine(dir, Mixtri.Core.Capture.VideoWriter.FinalizedMarkerName),
                Mixtri.Core.Capture.RecordingMarker.BuildContent(
                    Mixtri.Core.Capture.VideoWriter.EncoderVersion));
        }
        if (withPartial)
        {

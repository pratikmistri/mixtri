...
/// Set MIXTRI_PROBE_VIDEO to a real .mp4 to run; skips otherwise.
/// </summary>
/// <remarks>
/// This is an opt-in investigation harness, not a regression test: with MIXTRI_PROBE_VIDEO unset
...
public class PreviewSustainedRenderProbeTests
{
    private static string? ProbeVideo =>
        Environment.GetEnvironmentVariable("MIXTRI_PROBE_VIDEO") is { Length: > 0 } p && File.Exists(p) ? p : null;
    ...
    public async Task SustainedPlayback_AcrossCompositorRebuilds_DoesNotDegrade()
    {
        ...
        if (video is null) { Assert.Inconclusive("Set MIXTRI_PROBE_VIDEO to a real .mp4 to run."); return; }

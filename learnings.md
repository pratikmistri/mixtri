...
- **Grouping keys off `manifest.Project.CreatedAt`, NOT `RecentProject.LastUsedUtc`.** Last-used ordering made cards jump position simply because a project was opened; creation date is stable, and because it lives in the manifest it travels with the file, so a project copied from another machine still groups under the day it was recorded. Fallbacks, in order: manifest `CreatedAt` → `File.GetCreationTime` → the recents entry, so a package with an unreadable manifest lands in a sensible group instead of at `DateTime.MinValue`.
